...
/*   Updated: 2024/07/16 17:06:34 by shiori           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    return(y>=0 && y<game->map.y && x>=0 &&x<game->map.x &&game->map.data[y][x]!='1' &&visited[y][x]=='0');
}
...
int check_valid_path(t_game *game,int x,int y,char **visited)
{
    ...
    while(i<4)
    {
        ...
        if(is_valid_move(game,new_x,new_y,visited)&&check_valid_path(game,new_x,new_y,visited))
            return (1);
        i++;
    }
    return (0);
